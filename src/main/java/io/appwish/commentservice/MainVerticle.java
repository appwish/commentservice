...
import io.appwish.commentservice.repository.impl.PostgresCommentRepository;
import io.appwish.commentservice.repository.impl.Query;
import io.appwish.commentservice.verticle.DatabaseVerticle;
import io.appwish.commentservice.verticle.GrpcVerticle;
...
public class MainVerticle extends AbstractVerticle {
  ...
  public void start(final Promise<Void> startPromise) {
    ...
    retriever.getConfig(event -> {
      ...
      final CommentRepository repository = new PostgresCommentRepository(client);
      final EventBusConfigurer util = new EventBusConfigurer(vertx.eventBus());

      util.registerCodecs();

      // TODO It's here just for development purposes
      client.preparedQuery(Query.CREATE_COMMENTS_TABLE.sql(), query -> { });
  ...
  private Future<Void> deployDatabaseVerticle(final CommentRepository repository) {
    final Promise<Void> promise = Promise.promise();
